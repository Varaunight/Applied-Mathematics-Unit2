...
From Newton's Second Law, we know that, \TODO{Define in terms of momentum}
\begin{equation}
	F=ma \,.
\end{equation}
...
As force is a vector, we must be mindful that this value considers the direction from $v$ to $u$. Therefore, the true average force exerted in the direction of $u$ to $v$ is -(-300N) which is 300N. \TODO{Exaplin this as the Force ON THE WATER and the negative of the FIORCE OF THE WATER ON THE WALL}
...
As the string is inextensible, the speed of both particles must be the same. Therefore, we get that the Total Final KE of the system is \TODO{0.5mv squared in formula, aslo separate into v1 and v2; also cut down in Rep Math.},
\begin{align}
	KE_F & = KE_A + KE_B \nn \\
...
\textbf{\textit{Solve and Evaluate:}} \\ \\ \TODO{fix the solve and evaluate}
...
\textbf{\textit{Represent Mathematically:}} \\ \\ \TODO{See goodnotes:}
