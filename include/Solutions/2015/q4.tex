...
We want to determine the two unknowns $a$ and $b$. We can use two constraints, that from \rprop{mod2:prop:ContinuousRV:1}, 
\begin{equation}
...
In order to solve for $a$ and $b$, we must formulate another equation using the cumulative distribution function of $X$ given as $F(z)$. From \rdef{mod2:defn:ContinuousRV:CDF}, 
\begin{align}
...
From \rdef{mod2:note:ContinuousRV:CDF2}, \TODO{fix this reference} we can solve for $P(X>0.5)$ as follows,
\begin{align}
	P(X>0.5) & = 1 - F(0.5) \nn \\
	         & = 1 - 0.8 \nn \\
	         & = 0.2.
...
Similarly, in order to calculate $P(X \geq 10)$, we have to standardize in a similar manner as \req{2015:q4:Normal1}. This result follows as,
\begin{align}
...
We are asked to perform a $\chi^2$ goodness-of-fit test at the 5\% significance level. From Section(placeholder for CHI Squared notes)\TODO{Insert ref}, we know that,
\begin{equation}
