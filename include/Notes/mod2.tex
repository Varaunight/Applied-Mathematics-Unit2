...
\begin{defn} \label{mod2:note:NormalConvention}
	The convention for Normal Probability is the symbol $\phi$. $\phi(k)$ is defined as,
	\begin{equation}
		\phi(k) = P(Z<k).
	\end{equation}
\end{defn}
...
\begin{note}
	Some useful results for Normal Probability are,
	\begin{align}
		\phi(-k) & = 1 - \phi(k) \nn \\
		P(Z>k) & = 1 - \phi(k) \nn \\
		P(a<Z<b) & = \phi(b) - \phi(a).
...
Var[ag(X) + bf(Y)] &= a^2Var[g(X)] + b^2Var[f(Y)], \\
Var[ag(X) - bf(Y)] &= a^2Var[g(X)] + b^2Var[f(Y)].
\end{align}
