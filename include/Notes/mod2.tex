...
\begin{note}\label{mod2:note:DiscreteUniform}
	In the discrete case, the notation $X \sim \text{Unif}(a, b)$ or $X \sim \text{U}(a, b)$ implies that $X$ can take \textbf{integer} values $x$ such that $a \leq x \leq b$. Here, the total number of values $X$ can take is given by
	\begin{equation}
		n = b - a + 1,
	\end{equation}
...
\begin{defn}\label{mod2:defn:Geomtric}
	A discrete random variable $X$ is said to follow a \textbf{geometric distribution} with parameter $p$, $X \sim \text{Geo}(p)$, if its probability mass function, $P$, is given by
\begin{equation}
...
\section{Continuous Random Variables} \label{mod2:section:ContinuousRandomVar}
...
\begin{law} \label{mod2:law:UnconsciousStat}
	In the above formula for variance of a continuous random variable,  the \textbf{Law of the Unconscious Statistician (LOTUS)} is used. This law states that if $X$ is a continuous random variable with probability density function $f$, then for any function g,
	\begin{equation}
...
Define  functions $g$ and $f$ with $a, b, c \in \mathbb{R}$. For \textbf{ANY} random variables (discrete or continuous) $X$ and $Y$, the following properties hold
\begin{align}
E[aX + b] &= aE[X] + b, \\
E[X + Y] &= E[X] + E[Y], \\
E[ag(X) + bf(Y)  + c] &= aE[g(X) ]+ bE[f(Y)] + c, \\
\var[ag(X) + b] &= a^2\var[g(X)].
\end{align}
