...
\begin{defn} \label{mod2:defn:ContinuousRV:CDF}
	For a continuous random variable $X$ with p.d.f $f$, we define the \textbf{cumulative distribution function, F,} as follows,
	\begin{equation}
		F(x) = P(X \leq x), \
	\end{equation}
...
\begin{note}\label{mod2:note:NormalApproxtoPois:Continuity}
	...
	\begin{align}
	P(X=i) &= P \left( i -\frac{1}{2} < X < i + \frac{1}{2} \right), \\
